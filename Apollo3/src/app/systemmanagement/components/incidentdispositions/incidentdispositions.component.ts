...
@Component({
  templateUrl: 'incidentdispositions.component.html',
  standalone: true,
  selector: 'incidentdispositions',
  imports: [CommonModule, FormsModule],
  styleUrls: ['./incidentdispositions.css']
})
export class IncidentDispositionsComponent {
  ...
  loadData() {
    ...
    this.http.post<DispositionType[]>(`https://${this.config.systemURL.trim()}/API/Incidents/Incidents/get/dispotypes/modification`, this.authService.getSession()).subscribe(
      (response) => {
  ...
  createDisposition() {
    ...
    this.http.post(`https://${this.config.systemURL.trim()}/API/Incidents/Incidents/create/dispotypes/${newD}`, this.authService.getSession()).subscribe(
      (response) => {
  ...
  setDispositionInactive(dispo: string) {
    ...
    this.http.post(`https://${this.config.systemURL.trim()}/API/Incidents/Incidents/modify/dispotypes/${dispo.replaceAll('/', '|') }/N`, this.authService.getSession()).subscribe(
      (response) => {
  ...
  setDispositionActive(dispo: string) {
    ...
    this.http.post(`https://${this.config.systemURL.trim()}/API/Incidents/Incidents/modify/dispotypes/${dispo.replaceAll('/', '|') }/Y`, this.authService.getSession()).subscribe(
      (response) => {
        const dispoObj: DispositionType | null = this.dispos.find(d => d.disposition === dispo) ?? null;
        if (dispoObj == null) return;
        dispoObj.active = 'Y';
